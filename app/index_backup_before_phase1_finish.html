  ...
  <head>
    ...
    <style>
      ...
      * {
      ...
      .admin-create-fields .admin-create-title {
        font-size: 13px;
        font-weight: 600;
        color: #374151;
      }

      .admin-create-fields .admin-create-grid {
        display: grid;
        grid-template-columns: repeat(auto-fit, minmax(180px, 1fr));
        gap: 12px;
      }

      .admin-create-fields small {
        color: #6b7280;
        font-size: 12px;
      }
      .admin-optional {
      ...
      <main>
        ...
        <section class="section" id="registro">
          ...
          <form class="form" id="registro-form">
            ...
            <div id="adminCreateFields" class="admin-create-fields" aria-hidden="true">
              <div class="admin-create-title">Admin (opcional)</div>
              <div class="admin-create-grid">
                <div>
                  <label for="adminCreateCosto">Costo</label>
                  <input id="adminCreateCosto" type="number" step="0.01" min="0" placeholder="0" />
                </div>
                <div>
                  <label for="adminCreatePVenta">P.Venta override (opcional)</label>
                  <input id="adminCreatePVenta" type="number" step="0.01" min="0" placeholder="0" />
                </div>
              </div>
              <small>Solo visible en Modo Admin</small>
            </div>
      ...
      <table id="tablaPrendas" class="table">
        ...
        <tbody id="prendas-tbody">
          <tr id="loadingRow" data-db-loader>
            <td colspan="16" class="muted" style="padding:14px 16px;">Cargando…</td>
          </tr>
          <tr id="noResultsRow" data-empty-row style="display:none">
            <td colspan="16" class="muted" style="padding:14px 16px;">Sin resultados.</td>
          </tr>
</tbody>
      </table>
...
const auth = getAuth(app);
      ...
      const loadAdminCostMap = async (rows = []) => {
        ...
        for (const chunk of chunks) {
          const snap = await getDocs(
            query(collection(db, PRENDAS_ADMIN_COLLECTION), where("docId", "in", chunk))
          );
          snap.docs.forEach((docSnap) => {
            const data = docSnap.data() || {};
            const lookupId = safeString(data.docId || docSnap.id).trim();
            if (!lookupId) return;
            map.set(lookupId, {
      ...
      const createPrendaRow = (item) => {
        ...
        const row = document.createElement("tr");
        row.dataset.docId = safeString(item.docId || item.id).trim();
      ...
      registroForm.addEventListener("submit", async (event) => {
        ...
        try {
          ...
          const descripcion = [tipoNombre, proveedorNombre, colorNombre, tallaNombre]
            .filter(Boolean)
            .join(" ");
          const docId = normalizeSku(codigo);
          ...
          if (duplicateFound) {
            setRegistroStatus("Código actualizado en public/admin.", "warning");
          } else if (isAdminViewEnabled()) {
            setRegistroStatus("Prenda guardada correctamente en colecciones public/admin.", "success");
          } else {
            setRegistroStatus("Prenda guardada correctamente en colección public.", "success");
          }
        } catch (error) {
          const message =
...
^XZ`;
      ...
      const applyAdminUI = ({ refresh = true } = {}) => {
        ...
        if (adminCreateFields) {
          adminCreateFields.style.display = enabled ? "grid" : "none";
